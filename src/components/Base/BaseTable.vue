        ...
        <table class="base-table">
          ...
          <tbody>
            ...
            >
              ...
              >
                <slot :name="`cell-${col.key}`" :row="row" :val="row[col.key]" :index="rowIndex">
                  {{ row[col.key] }}
                </slot>
              </td>
      ...
      <div class="table-footer" v-if="pagination">
        <div class="d-flex items-center gap-4">
             <div class="footer-info">
              Showing <span class="highlight">{{ paginationStart }}-{{ paginationEnd }}</span> of <span class="highlight">{{ pagination.total }}</span> results
            </div>
...
const props = defineProps({
  ...
  pagination: { 
    type: Object as () => { currentPage: number, total: number, perPage: number, lastPage?: number } | null, 
    default: null 
  },
  searchQuery: { type: String, default: '' },
  selectedRows: { type: Array as () => any[], default: () => [] },
  showOptions: { type: Array as () => number[], default: () => [5, 10, 25, 50, 100] },
...
watch(() => props.columns, () => {
   ...
   hiddenColumns.value = hiddenColumns.value.filter(k => props.columns.find(c => c.key === k));
}, { deep: true });
...
const totalPages = computed(() => {
  ...
  return props.pagination.lastPage || Math.ceil(props.pagination.total / props.pagination.perPage);
});
...
.base-table {
  ...
  thead {
    th {
      ...
      &.checkbox-cell { width: 3rem; text-align: center; padding-left: 2rem; }
      &.actions-cell { width: 3rem; }
      ...
      .sort-icon { font-size: 1rem; opacity: 0.5; }
      
      &.text-right { text-align: right; .th-content { justify-content: flex-end; } }
      &.text-center { text-align: center; .th-content { justify-content: center; } }
    }
  }
  ...
  tbody {
    ...
    td {
      ...
      &.checkbox-cell { width: 3rem; text-align: center; padding-left: 2rem; }
      &.actions-cell { width: 3rem; text-align: right; padding-right: 2rem; }
